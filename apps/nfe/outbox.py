...
# EX-POL / double finalize: no máximo 1 outbox por (tenant, tipo, invoice)
_IDEMPOTENT_LIFECYCLE = frozenset(
    {
        EVENT_AUTHORIZED,
        EVENT_REJECTED,
        EVENT_CANCELLED,
        EVENT_POLL_EXHAUSTED,
    }
)
...
def _payload(invoice: NfeInvoice, *, extra: dict[str, Any] | None = None) -> dict[str, Any]:
    body: dict[str, Any] = {
        ...
        "denegada": bool((invoice.last_validation or {}).get("denegada")),
    }
    if extra:
        body.update(extra)
    return body
...
def publish_nfe_lifecycle_event(
    ...
    *,
    ...
    """Enfileira outbox (dispatch on_commit). aggregate=nfe_invoice. Idempotente em ciclo de vida."""
    if event_type in _IDEMPOTENT_LIFECYCLE:
        exists = OutboxMessage.objects.filter(
